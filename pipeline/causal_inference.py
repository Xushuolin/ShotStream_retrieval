...
class CausalInferencePipeline(FrameConcatCausalModel):
    def __init__(
            self,
            args,
            device,
        ...
        self.local_attn_size =getattr(self.args, "local_attn_size", 
                getattr(self.args, "model_kwargs", {}).get("local_attn_size", -1))
        
        self.sink_size = getattr(self.args, "sink_size", 
    ...
    def _kv_retrieval_topk_context(
        ...
        retrieval_shot_flags = torch.tensor(
            self._to_local_context_flags(candidate_shot_flags), dtype=torch.int32, device=device
        )
        self.generator(
        ...
        selected_shot_flags = self._to_local_context_flags(selected_shot_flags)
    ...
    def inference(
        ...
        shots_captions = batch['shots_captions']  # batch['shots_captions'][0][0][-1][0]
        ...
        print(f"[DEBUG] video_data shape is {video_data.shape}")
        with torch.no_grad():
            ...
            video_latents = self.vae.encode_to_latent(video_data).to(device).to(dtype)  # [1, f, c, h, w]
            print(f"[DEBUG] video_latent shape is {video_latents.shape}")

            if video_latents.shape[1] % self.num_frame_per_block != 0:
                video_latents = video_latents[::, :(video_latents.shape[1] // self.num_frame_per_block) * self.num_frame_per_block]
                print(f"Generation shape is {video_latents.shape}")
        ...
        local_attn_cfg = -1  # use large cache
