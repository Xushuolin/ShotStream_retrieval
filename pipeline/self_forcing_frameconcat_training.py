...
class SelfForcingFrameconcatTrainingPipeline:
    def __init__(self,
        ...
        self.num_frame_per_block = num_frame_per_block  # 3
        self.context_noise = context_noise  # 0
        self.i2v = False

        self.kv_cache1 = None
        self.kv_cache2 = None
        self.crossattn_cache = None
        self.independent_first_frame = independent_first_frame  # False
        ...
        def _resolve_kv_frames(local_cfg):
            if isinstance(local_cfg, (list, tuple)):
                ...
                base = int(local_cfg)
                if base == -1:
                    print(f"Set KV Cache length into 21")
                    return 21
                return min(base + slice_last_frames, num_training_frames)
        ...
        self.kv_cache_size = int(kv_frames) * self.frame_seq_length  # token_number, kv_frames * 1560
        print(f"[KV policy] local_attn_size={self.local_attn_size} slice_last_frames={slice_last_frames} num_training_frames={num_training_frames} -> kv_frames={kv_frames}")
    ...
    def inference_with_trajectory(
            ...
            context_latent: Optional[torch.Tensor] = None,  # add for condition latent
        ...
        # [NOTE] Initialize KV Context cache
        condition_frame_numbers = context_latent.shape[1]
        ...
        if context_latent.shape[1] > 0:
            with torch.no_grad():
                self.generator(
                    ...
                    shot_flags_for_rope=shot_flags_for_rope[:context_latent.shape[1]],
                    ###
                    current_start=0,
                )
        ...
        all_num_frames = [self.num_frame_per_block] * num_blocks  # [3, 3, 3, 3, 3, 3, 3]
        if self.independent_first_frame and initial_latent is None:
            all_num_frames = [1] + all_num_frames
        num_denoising_steps = len(self.denoising_step_list)
        exit_flags = self.generate_and_sync_list(len(all_num_frames), num_denoising_steps, device=noise.device)  # sample from 0-step, number is chunk number, shape is [chunk_number]
        ...
        for block_index, current_num_frames in enumerate(all_num_frames):
            ...
            for index, current_timestep in enumerate(self.denoising_step_list):
                # print(f"Denoise index is {index}, current timestep is {current_timestep}")
                # If scheduled, set local_attn_size dynamically per timestep
                if isinstance(self.local_attn_size, (list, tuple)):
        ...
        return output, denoised_timestep_from, denoised_timestep_to  # return pred_x0
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device):
        ...
        for _ in range(self.num_transformer_blocks):
            kv_cache1.append({
                # "k": torch.zeros([batch_size, self.kv_cache_size, 12, 128], dtype=dtype, device=device),  # [b, num_tokens, head, channel]
                # "v": torch.zeros([batch_size, self.kv_cache_size, 12, 128], dtype=dtype, device=device),
                "k": torch.zeros([batch_size, self.kv_cache_size, self.num_heads, self.d], dtype=dtype, device=device),
                "v": torch.zeros([batch_size, self.kv_cache_size, self.num_heads, self.d], dtype=dtype, device=device),
                "global_end_index": torch.tensor([0], dtype=torch.long, device=device),
                "local_end_index": torch.tensor([0], dtype=torch.long, device=device)
            })
