...
def rope_params_angle(max_seq_len, dim, theta=10000):
    assert dim % 2 == 0
    freqs = torch.outer(
        torch.arange(max_seq_len),
        1.0 / torch.pow(theta,
...
def rope_apply_dynamic(x, grid_sizes, freqs, shot_flags_for_rope, theta=1/6):
    # print(f"[DEBUG] shot_flags is {shot_flags_for_rope}")
    n, c = x.size(2), x.size(3) // 2
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        ...
        x_i = torch.view_as_complex(x[i, :seq_len].to(torch.float64).reshape(
            seq_len, n, -1, 2))
        freqs_i = torch.cat([
            (freqs[0][:f].view(f, 1, 1, -1) + shot_flags_for_rope.view(f, 1, 1, -1) * theta).expand(f, h, w, -1),
            freqs[1][:h].view(1, h, 1, -1).expand(f, h, w, -1),
            freqs[2][:w].view(1, 1, w, -1).expand(f, h, w, -1)
        ],
            dim=-1).reshape(seq_len, 1, -1)
...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, freqs_dynamic=None, shot_flags_for_rope=None):
        r"""
        Args:
        ...
        if shot_flags_for_rope is None:
            ...
            x = flash_attention(
                q=rope_apply_dynamic(q, grid_sizes, freqs_dynamic, shot_flags_for_rope),
                k=rope_apply_dynamic(k, grid_sizes, freqs_dynamic, shot_flags_for_rope),
                v=v,
                k_lens=seq_lens,
                window_size=self.window_size)
...
class WanT2VCrossAttention(WanSelfAttention):
    ...
    def forward(
        ...
        if context.shape[0] == 1:        
            ...
            # compute query, key, value
            assert x.shape[0] == 1, print(f"Cross Attention Could Only Support bs=1")
            f, h, w = grid_sizes[0][0].item(), grid_sizes[0][1].item(), grid_sizes[0][2].item()
            q = self.norm_q(self.q(x)).view(f, h*w, n, d)

            if crossattn_cache is not None:
                if not crossattn_cache["is_init"]:
                    crossattn_cache["is_init"] = True
                    k = self.norm_k(self.k(context)).view(context.shape[0], context.shape[1], n, d)
                    v = self.v(context).view(context.shape[0], context.shape[1], n, d)
                    crossattn_cache["k"] = k
                    crossattn_cache["v"] = v
                else:
                    k = crossattn_cache["k"]
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if shot_flags_for_rope is not None:
            ...
            freqs, freqs_dynamic=freqs_dynamic, shot_flags_for_rope=shot_flags_for_rope)
        else:
            y = self.self_attn(
                self.norm1(x) * (1 + e[1]) + e[0], seq_lens, grid_sizes,
                freqs)
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def _forward(
        ...
        # embeddings
        x = [self.patch_embedding(u.unsqueeze(0)) for u in x]  # torch.Size([1, 16, 31, 60, 104]) [b c f h w] -> [b, c, f, h, w]
        grid_sizes = torch.stack(
            [torch.tensor(u.shape[2:], dtype=torch.long) for u in x])
        x = [u.flatten(2).transpose(1, 2) for u in x]  # [b, f h w, c]
        seq_lens = torch.tensor([u.size(1) for u in x], dtype=torch.long)
        seq_len = seq_lens.max()  # add for dynamic length
        assert seq_lens.max() <= seq_len
        assert len(x) == 1  # [Add] hard code, can only perform bs =1
        x = torch.cat([
